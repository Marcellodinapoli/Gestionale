/**
 * Prompt predefinito roleplay (allineato a BK / CreditCalc).
 * Usato solo se il documento Firestore `roleplay/{id}` non ha `prompt` né `gptPrompt`.
 * In produzione il testo ufficiale è quello salvato dal backoffice su ogni simulazione.
 */
export const DEFAULT_ROLEPLAY_SIMULATION_PROMPT = `PROMPT ROLEPLAY – SIMULAZIONE RECUPERO CREDITI
