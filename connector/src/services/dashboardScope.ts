...
export type ScopeBindResult = {
  where: string;
  join: string;
  praticaAlias: string;
};

type ScopeInput = HomeScopeFilter & { tenantId: string };

export function bindPraticaScope(req: sql.Request, input: ScopeInput, alias = "p"): ScopeBindResult {
  ...
  if (input.mode === "operator" && input.userId) {
    ...
    clauses.push(
      `${alias}.AssegnatarioId IN (${input.memberIds.map((_, i) => `@mem${i}`).join(", ")})`
    );
  }
  ...
  if (input.perimetroOr?.length) {
    ...
    input.perimetroOr.forEach((pair, i) => {
      ...
      if (pair.numeriMandante?.length) {
        ...
        orParts.push(
          `(${alias}.MandanteId = @pm${i} AND ${alias}.NumeroMandante IN (${pair.numeriMandante
            .map((_, j) => `@pmn${i}_${j}`)
            .join(", ")}))`
        );
      } else {
