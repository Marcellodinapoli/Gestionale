...
export async function appendAuditLog(
  ...
  if (input.userId) req.input("userId", sql.UniqueIdentifier, input.userId);
  req.input("action", sql.NVarChar(100), input.action);
  req.input("entity", sql.NVarChar(100), input.entity);
  if (input.entityId) req.input("entityId", sql.NVarChar(100), input.entityId);
  if (input.dettaglio) req.input("metadata", sql.NVarChar(sql.MAX), input.dettaglio);
