...
function mapPrismaToSql(data: Record<string, unknown>): Record<string, unknown> {
...
export async function listMandanti(cfg: ConnectorConfig["db"], req: MandanteListRequest) {
  const pool = await getPool(cfg);
  const where = bindFilter(pool.request(), req.tenantId, req.filter);
  const orderCol =
    req.orderBy === "ragioneSociale" ? "m.RagioneSociale" : "m.Codice";
...
export async function countMandanti(
...
export async function getMandanteById(
  cfg: ConnectorConfig["db"],
  tenantId: string,
  id: string,
  includePraticaCount = false
) {
  const pool = await getPool(cfg);
...
export async function createMandante(
  cfg: ConnectorConfig["db"],
  tenantId: string,
  ...
  cols.forEach((c, i) => {
    const val = mapped[c];
    if (c.endsWith("Id") || c === "TenantId") req.input(`p${i}`, sql.UniqueIdentifier, val);
    else if (typeof val === "number") req.input(`p${i}`, sql.Float, val);
    else req.input(`p${i}`, sql.NVarChar(sql.MAX), val == null ? null : String(val));
  });
  ...
  if (row) row.Perimetri = row.PerimetriJson ?? row.Perimetri;
  return row;
}

export async function updateMandante(
  cfg: ConnectorConfig["db"],
  tenantId: string,
  id: string,
  data: Record<string, unknown>
  ...
  if (!sets.length) return getMandanteById(cfg, tenantId, id);
  const result = await req.query(`
...
export async function deleteMandante(cfg: ConnectorConfig["db"], tenantId: string, id: string) {
  const pool = await getPool(cfg);
  ...
  if (linked > 0) {
    const err = new Error(`Impossibile eliminare: ${linked} pratiche collegate`) as Error & {
      statusCode?: number;
    };
    err.statusCode = 409;
    throw err;
