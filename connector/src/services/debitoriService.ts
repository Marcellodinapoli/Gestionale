...
function mapDebitoreToSql(data: Record<string, unknown>): Record<string, unknown> {
...
export async function listDebitori(cfg: ConnectorConfig["db"], req: DebitoreListRequest) {
...
export async function idsByCodiceFiscale(
...
export async function getDebitoreById(cfg: ConnectorConfig["db"], tenantId: string, id: string) {
  const pool = await getPool(cfg);
...
export async function createDebitore(
  ...
  cols.forEach((c, i) => {
    const val = mapped[c];
    if (c.endsWith("Id")) req.input(`p${i}`, sql.UniqueIdentifier, val);
    else req.input(`p${i}`, sql.NVarChar(500), val == null ? null : String(val));
  });
  const result = await req.query(`
...
export async function updateDebitore(
  cfg: ConnectorConfig["db"],
  tenantId: string,
  id: string,
  data: Record<string, unknown>
  ...
  for (const [col, val] of Object.entries(mapped)) {
    if (col === "TenantId" || col === "Id") continue;
    req.input(`u${i}`, sql.NVarChar(500), val == null ? null : String(val));
    sets.push(`${col} = @u${i}`);
    i++;
  }
  if (!sets.length) return getDebitoreById(cfg, tenantId, id);
  const result = await req.query(`
...
export async function deleteDebitore(cfg: ConnectorConfig["db"], tenantId: string, id: string) {
...
export async function listRecapiti(cfg: ConnectorConfig["db"], debitoreId: string) {
  const pool = await getPool(cfg);
  const result = await pool
    .request()
...
export async function countRecapiti(
  cfg: ConnectorConfig["db"],
  debitoreId: string,
  tipo?: string
) {
...
export async function findFirstRecapito(
  cfg: ConnectorConfig["db"],
  filter: { id?: string; debitoreId?: string; tipo?: string }
) {
  const pool = await getPool(cfg);
  const req = pool.request();
...
export async function createRecapito(
  cfg: ConnectorConfig["db"],
  data: { debitoreId: string; tipo: string; valore: string; ordine?: number; stato?: string | null }
) {
  const pool = await getPool(cfg);
  const result = await pool
    .request()
...
export async function updateRecapito(
  cfg: ConnectorConfig["db"],
  id: string,
  data: Record<string, unknown>
) {
  ...
  if (!sets.length) return findFirstRecapito(cfg, { id });
  const result = await req.query(`
...
export async function deleteRecapito(cfg: ConnectorConfig["db"], id: string) {
  await getPool(cfg).then((pool) =>
    pool.request().input("id", sql.UniqueIdentifier, id).query(`DELETE FROM dbo.DebitoreRecapiti WHERE Id = @id`)
  );
}

export async function deleteRecapitiByDebitore(cfg: ConnectorConfig["db"], debitoreId: string) {
  await getPool(cfg).then((pool) =>
    pool
      .request()
      .input("debitoreId", sql.UniqueIdentifier, debitoreId)
