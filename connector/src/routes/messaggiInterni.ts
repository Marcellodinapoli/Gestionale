...
import {
  createMessaggiInterni,
  deleteMessaggioInterno,
  deleteMessaggiInterniByPratica,
  getMessaggioInternoById,
  listMessaggiInterni,
  updateMessaggioInternoLetto,
  updateMessaggioInternoTesto,
} from "../services/messaggiInterniService.js";

export function createMessaggiInterniRouter(cfg: ConnectorConfig) {
  const router = Router({ mergeParams: true });
  const resolveTenant = createTenantResolver(cfg);

  router.post("/list", resolveTenant, async (req, res, next) => {
    try {
      const items = await listMessaggiInterni(
        cfg.db,
        req.tenant!.tenantId,
        req.body?.filter,
        req.body?.take ?? 100
  ...
  router.get("/:id", resolveTenant, async (req, res, next) => {
    try {
      const item = await getMessaggioInternoById(
        cfg.db,
        req.tenant!.tenantId,
  ...
  router.post("/:id/letto", resolveTenant, async (req, res, next) => {
    try {
      await updateMessaggioInternoLetto(
        cfg.db,
        req.tenant!.tenantId,
        String(req.params.id),
        Boolean(req.body?.letto ?? true)
      );
      res.json({ ok: true });
    } catch (err) {
      next(err);
  ...
  router.patch("/:id", resolveTenant, async (req, res, next) => {
    try {
      await updateMessaggioInternoTesto(
        cfg.db,
        req.tenant!.tenantId,
        String(req.params.id),
  ...
  router.delete("/pratica/:praticaId", resolveTenant, async (req, res, next) => {
    try {
      await deleteMessaggiInterniByPratica(
        cfg.db,
        req.tenant!.tenantId,
