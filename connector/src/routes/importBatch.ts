...
import {
  countPraticheImportBatch,
  createImportBatch,
  deleteImportBatch,
  findImportBatchByLotKey,
  getImportBatchById,
...
export function createImportBatchRouter(cfg: ConnectorConfig) {
  ...
  router.patch("/:id", resolveTenant, async (req, res, next) => {
    try {
      const item = await updateImportBatch(
        cfg.db,
        req.tenant!.tenantId,
  ...
  router.delete("/pratica/:praticaId", resolveTenant, async (req, res, next) => {
    try {
      await deletePraticaForImport(cfg.db, req.tenant!.tenantId, String(req.params.praticaId));
      res.json({ ok: true });
    } catch (err) {
      next(err);
    }
