...
import {
  ...
  releaseLock,
  releaseAllUserLocks,
  releaseLockForPratica,
  renewLock,
  findActiveLocksByPraticaIds,
  LOCK_TTL_SEC,
} from "../services/lockService.js";

const STREAM_POLL_MS = 5_000;

function requireUserId(req: { body?: { userId?: string }; query?: { userId?: string } }) {
...
function statusJson(status: Awaited<ReturnType<typeof getLockStatus>>) {
  return {
    owned: status.owned,
...
export function createLockRouter(cfg: ConnectorConfig) {
