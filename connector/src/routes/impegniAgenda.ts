...
import {
  completeImpegnoAgenda,
  createImpegnoAgenda,
  deleteImpegnoAgenda,
  getImpegnoAgendaById,
  listImpegniAgenda,
  updateImpegnoAgenda,
} from "../services/impegniAgendaService.js";

export function createImpegniAgendaRouter(cfg: ConnectorConfig) {
  const router = Router({ mergeParams: true });
  const resolveTenant = createTenantResolver(cfg);

  router.post("/list", resolveTenant, async (req, res, next) => {
    try {
      const items = await listImpegniAgenda(
        cfg.db,
        req.tenant!.tenantId,
        req.body?.filter,
        req.body?.take ?? 200
  ...
  router.patch("/:id", resolveTenant, async (req, res, next) => {
    try {
      ...
      const item = await updateImpegnoAgenda(
        cfg.db,
        req.tenant!.tenantId,
