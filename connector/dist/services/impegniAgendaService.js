...
function bindFilter(req, tenantId, filter) {
    ...
    if (filter?.completato === false)
        clauses.push("i.Completato = 0");
    if (filter?.completato === true)
        clauses.push("i.Completato = 1");
    if (filter?.memoAtGte) {
...
export async function listImpegniAgenda(cfg, tenantId, filter, take = 200) {
...
export async function getImpegnoAgendaById(cfg, tenantId, id) {
    const rows = await listImpegniAgenda(cfg, tenantId, { id }, 1);
    return rows[0] ?? null;
}
export async function createImpegnoAgenda(cfg, tenantId, data) {
    const pool = await getPool(cfg);
...
export async function completeImpegnoAgenda(cfg, tenantId, id, userId) {
    const pool = await getPool(cfg);
    await pool
        .request()
        .input("tenantId", sql.UniqueIdentifier, tenantId)
...
export async function updateImpegnoAgenda(cfg, tenantId, id, userId, data) {
    const pool = await getPool(cfg);
    const sets = [];
    const req = pool.request()
        .input("tenantId", sql.UniqueIdentifier, tenantId)
...
export async function deleteImpegnoAgenda(cfg, tenantId, id, userId) {
    const pool = await getPool(cfg);
    await pool
        .request()
        .input("tenantId", sql.UniqueIdentifier, tenantId)
