...
async function verifyPratica(pool, tenantId, praticaId) {
...
function purgeExpiredSql(alias) {
 ...
 */
export async function acquireLock(cfg, tenantId, praticaId, userId) {
...
export async function renewLock(cfg, tenantId, praticaId, userId) {
    const pool = await getPool(cfg);
    if (!(await verifyPratica(pool, tenantId, praticaId))) {
        throw new Error("Pratica non trovata");
    }
...
export async function releaseLock(cfg, tenantId, praticaId, userId) {
    const pool = await getPool(cfg);
    await pool
        .request()
        .input("tenantId", sql.UniqueIdentifier, tenantId)
...
export async function releaseAllUserLocks(cfg, tenantId, userId) {
    const pool = await getPool(cfg);
    await pool
        .request()
        .input("tenantId", sql.UniqueIdentifier, tenantId)
...
export async function releaseLockForPratica(cfg, tenantId, praticaId) {
    const pool = await getPool(cfg);
    await pool
        .request()
        .input("tenantId", sql.UniqueIdentifier, tenantId)
...
export async function getLockStatus(cfg, tenantId, praticaId, userId) {
    const pool = await getPool(cfg);
...
export async function findActiveLocksByPraticaIds(cfg, tenantId, praticaIds) {
...
export async function purgeExpiredLocksBatch(cfg) {
    const pool = await getPool(cfg);
