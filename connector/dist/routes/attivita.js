...
import { countAttivita, createAttivita, deleteManyAttivita, getAttivitaById, groupByUserIdAttivita, listAttivita, toggleFissaAttivita, updateAttivita, updateManyAttivita, } from "../services/attivitaService.js";
export function createAttivitaRouter(cfg) {
    ...
    router.post("/update-many", resolveTenant, async (req, res, next) => {
        try {
            const result = await updateManyAttivita(cfg.db, req.tenant.tenantId, req.body?.filter ?? {}, req.body?.data ?? {});
            res.json(result);
        }
        catch (err) {
            next(err);
    ...
    router.post("/toggle-fissa", resolveTenant, async (req, res, next) => {
        try {
            await toggleFissaAttivita(cfg.db, req.tenant.tenantId, String(req.body?.attivitaId), String(req.body?.praticaId), Boolean(req.body?.fissata));
            res.json({ ok: true });
        }
        catch (err) {
            next(err);
    ...
    router.patch("/:id", resolveTenant, async (req, res, next) => {
        try {
            const item = await updateAttivita(cfg.db, req.tenant.tenantId, String(req.params.id), req.body ?? {});
            if (!item) {
                res.status(404).json({ error: "Attività non trovata" });
                return;
            }
