...
import { assignPratica, canAccessPraticaSql, countPratiche, createPratica, deletePratica, getPraticaById, getPraticaRelations, groupByNumeroMandante, idsAffidoTemporaneo, idsImportoTotale, idsTotIncassato, listPratiche, nextNumeroPratica, searchPratiche, updatePratica, updateStatoPratica, } from "../services/praticheService.js";
export function createPraticheRouter(cfg) {
    ...
    router.post("/ids-importo-totale", resolveTenant, async (req, res, next) => {
        try {
            const body = req.body ?? {};
            const ids = await idsImportoTotale(cfg.db, req.tenant.tenantId, body.da != null ? Number(body.da) : undefined, body.a != null ? Number(body.a) : undefined);
            res.json({ ids });
        }
        catch (err) {
            next(err);
    ...
    router.post("/ids-tot-incassato", resolveTenant, async (req, res, next) => {
        try {
            const body = req.body ?? {};
            const ids = await idsTotIncassato(cfg.db, req.tenant.tenantId, body.da != null ? Number(body.da) : undefined, body.a != null ? Number(body.a) : undefined);
            res.json({ ids });
        }
        catch (err) {
            next(err);
    ...
    router.post("/can-access", resolveTenant, async (req, res, next) => {
        try {
            ...
            const ok = await canAccessPraticaSql(cfg.db, scope, String(body.praticaId || ""), Array.isArray(body.linkedIds) ? body.linkedIds.map(String) : undefined);
            res.json({ ok });
        }
        catch (err) {
            next(err);
    ...
    router.post("/:id/stato", resolveTenant, async (req, res, next) => {
        try {
            const body = req.body ?? {};
            const item = await updateStatoPratica(cfg.db, req.tenant.tenantId, req.params.id, String(body.stato || ""), body.promessaAt ? new Date(body.promessaAt) : body.promessaAt === null ? null : undefined);
            res.json({ item });
        }
        catch (err) {
            next(err);
